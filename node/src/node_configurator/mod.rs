...
pub const WALLET_PASSWORD_HELP: &str =
    "A password or phrase to encrypt your consuming wallet in the PrometheusNode database or decrypt a keystore file. Can be changed \
...
pub fn app_head() -> App<'static, 'static> {
    App::new("PrometheusNode")
        ...
        .author("Substratum, The-Dreadnought")
...
pub fn data_directory_default(dirs_wrapper: &dyn DirsWrapper, chain_name: &'static str) -> String {
    match dirs_wrapper.data_dir() {
        Some(path) => path.join("Prometheus").join(chain_name),
        None => PathBuf::from(""),
    }
    .to_str()
