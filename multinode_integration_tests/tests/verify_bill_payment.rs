...
use multinode_integration_tests_lib::prometheus_real_node::{
    ConsumingWalletInfo, EarningWalletInfo, NodeStartupConfigBuilder, PrometheusRealNode,
};
...
fn verify_bill_payment() {
    let mut cluster = match PrometheusNodeCluster::start() {
        Ok(cluster) => cluster,
        Err(_) => panic!(""),
    };
    ...
    let project_root = PrometheusNodeUtils::find_project_root();
    let (consuming_node_name, consuming_node_index) = cluster.prepare_real_node(&consuming_config);
    let consuming_node_path =
        PrometheusRealNode::node_home_dir(&project_root, &consuming_node_name);
    ...
    let serving_node_1_path =
        PrometheusRealNode::node_home_dir(&project_root, &serving_node_1_name);
    ...
    let serving_node_2_path =
        PrometheusRealNode::node_home_dir(&project_root, &serving_node_2_name);
    ...
    let serving_node_3_path =
        PrometheusRealNode::node_home_dir(&project_root, &serving_node_3_name);
