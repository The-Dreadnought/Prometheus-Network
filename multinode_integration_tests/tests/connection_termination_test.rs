...
use multinode_integration_tests_lib::prometheus_mock_node::PrometheusMockNode;
use multinode_integration_tests_lib::prometheus_node::{
    PortSelector, PrometheusNode, PrometheusNodeUtils,
};
use multinode_integration_tests_lib::prometheus_node_cluster::PrometheusNodeCluster;
use multinode_integration_tests_lib::prometheus_node_server::PrometheusNodeServer;
use multinode_integration_tests_lib::prometheus_real_node::PrometheusRealNode;
...
fn downed_nodes_not_offered_in_passes_or_introductions() {
    ...
    let mut cluster = PrometheusNodeCluster::start().unwrap();
    let (_, prometheus_real_node, mut node_map) = construct_neighborhood(&mut cluster, db, vec![]);
    ...
    // Debut a new Node
    debuter_node.transmit_debut(&prometheus_real_node).unwrap();
...
fn create_neighborhood(
...
) -> (PrometheusRealNode, PrometheusMockNode, PublicKey) {
    ...
    let (_, prometheus_real_node, mut node_map) = construct_neighborhood(cluster, db, vec![]);
    let prometheus_mock_node = node_map.remove(mock_node.public_key()).unwrap();
    (
        prometheus_real_node,
        prometheus_mock_node,
...
fn create_request_icp(
    originating_node: &PrometheusMockNode,
    exit_node: &PrometheusRealNode,
    stream_key: StreamKey,
    return_route_id: u32,
    server: &PrometheusNodeServer,
    chain_id: u8,
...
fn create_server_drop_report(
    exit_node: &PrometheusMockNode,
    originating_node: &PrometheusRealNode,
    stream_key: StreamKey,
    return_route_id: u32,
) -> IncipientCoresPackage {
...
fn ensure_no_further_traffic(mock_node: &PrometheusMockNode, masquerader: &dyn Masquerader) {
...
fn wait_for_client_shutdown(real_node: &PrometheusRealNode) {
...
fn wait_for_server_shutdown(real_node: &PrometheusRealNode, server_port: u16) {
    ...
    PrometheusNodeUtils::wrote_log_containing(
        ...
        &format!(
            ...
            PrometheusNodeCluster::host_ip_addr(),
            server_port
        ),
        Duration::from_secs(1),
    );
