...
pub trait PrometheusNode: Any {
    ...
    // If this PrometheusNode has a CryptDENull instead of a CryptDEReal, you can get it here.
    fn cryptde_null(&self) -> Option<&CryptDENull>;
    // The CryptDE that can be used for signing for this Node, if any. (None if it's a PrometheusRealNode with a CryptDEReal.)
    fn signing_cryptde(&self) -> Option<&dyn CryptDE>;
    // A reference to this PrometheusNode's public key.
...
pub struct PrometheusNodeUtils {}

impl PrometheusNodeUtils {
    ...
    pub fn wrote_log_containing(name: &str, pattern: &str, timeout: Duration) {
        ...
        while Instant::now() < time_limit {
            entire_log = PrometheusNodeUtils::retrieve_logs(name);
    ...
    pub fn retrieve_logs(name: &str) -> String {
        let mut command = Command::new(
            ...
            Command::strings(vec![
                ...
                name,
                "cat",
                "/node_root/home/PrometheusNode_rCURRENT.log",
            ]),
        );
