...
use crate::prometheus_mock_node::PrometheusMockNode;
...
/// Construct a neighborhood for testing that corresponds to a provided NeighborhoodDatabase, with a PrometheusRealNode
/// corresponding to the root of the database, PrometheusMockNodes corresponding to all the root's immediate neighbors,
/// and PrometheusMockNodes or fictional Nodes corresponding to all other Nodes in the provided database.
///
/// The result of this function is a single PrometheusRealNode and a series of PrometheusMockNodes, where the real Node
...
/// different NodeAddrs.
///
/// Of course, all the PrometheusNodes produced by this function will use CryptDENull, so any other PrometheusNodes
/// that are intended to communicate with them must use CryptDENull as well.
///
/// # Arguments
///
/// * `cluster` - A mutable reference to the PrometheusNodeCluster that should be used to create PrometheusRealNodes and PrometheusMockNodes.
/// * `model_db` - The database describing the structure that should be imposed on the
///                     PrometheusRealNode's internal database. This database is consumed by this call.
/// * `additional_keys_to_mock` - If this collection is empty, PrometheusMockNodes will only be created to correspond to
///                                 immediate neighbors of `model_db.root()`. If you want PrometheusMockNodes corresponding
...
///                             closely as possible the database that the PrometheusRealNode will have internally when
///                             `construct_neighborhood()` returns.
/// * `PrometheusRealNode` - The real Node corresponding to the NodeRecord at `model_db.root()`. It will have the same
///                             public key as the original `model_db.root()`, but a different NodeAddr.
/// * `HashMap<PublicKey, PrometheusMockNode>` The mock Nodes corresponding to other NodeRecords in `model_db`. They
...
pub fn construct_neighborhood(
    ...
    PrometheusRealNode,
    HashMap<PublicKey, PrometheusMockNode>,
) {
...
fn make_mock_node_map(
    cluster: &mut PrometheusNodeCluster,
    model_db: &NeighborhoodDatabase,
    real_node: &PrometheusRealNode,
    additional_keys_to_mock: Vec<&PublicKey>,
) -> (HashMap<PublicKey, PrometheusMockNode>, Vec<PublicKey>) {
    ...
    let mut mock_node_map = adjacent_mock_nodes
        ...
        .collect::<HashMap<PublicKey, PrometheusMockNode>>();
...
fn make_modified_node_records(
    model_db: NeighborhoodDatabase,
    mock_node_map: &HashMap<PublicKey, PrometheusMockNode>,
) -> Vec<NodeRecord> {
    model_db
        .keys()
        .iter()
...
fn make_and_send_final_setup_gossip(
    gossip_source_mock_node: &PrometheusMockNode,
    modified_nodes: &Vec<NodeRecord>,
    real_node: &PrometheusRealNode,
) {
...
fn form_mock_node_skeleton(
    ...
    model_db
        ...
        .collect::<Vec<PrometheusMockNode>>()
}
...
impl From<&PrometheusMockNode> for NodeRecord {
    fn from(mock_node: &PrometheusMockNode) -> Self {
        from_prometheus_node_to_node_record(mock_node)
    }
}

impl From<&PrometheusRealNode> for NodeRecord {
    fn from(real_node: &PrometheusRealNode) -> Self {
        from_prometheus_node_to_node_record(real_node)
    }
}

fn from_prometheus_node_to_node_record(prometheus_node: &dyn PrometheusNode) -> NodeRecord {
    let agr = AccessibleGossipRecord::from(prometheus_node);
