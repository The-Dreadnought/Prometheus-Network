...
use crate::prometheus_node::{PrometheusNode, PrometheusNodeUtils};
use crate::prometheus_real_node::NodeStartupConfig;
...
pub struct PrometheusNodeCluster {
    startup_configs: HashMap<(String, usize), NodeStartupConfig>,
    real_nodes: HashMap<String, PrometheusRealNode>,
    mock_nodes: HashMap<String, PrometheusMockNode>,
    host_node_parent_dir: Option<String>,
    next_index: usize,
    pub chain_id: u8,
}

impl PrometheusNodeCluster {
    pub fn start() -> Result<PrometheusNodeCluster, String> {
        PrometheusNodeCluster::cleanup()?;
        PrometheusNodeCluster::create_network()?;
        ...
        if Self::is_in_jenkins() {
            PrometheusNodeCluster::interconnect_network()?;
        }
        Ok(PrometheusNodeCluster {
    ...
    pub fn prepare_real_node(&mut self, config: &NodeStartupConfig) -> (String, usize) {
        ...
        let name = PrometheusRealNode::make_name(index);
        self.startup_configs
            .insert((name.clone(), index), config.clone());
        PrometheusRealNode::prepare(&name);

        (name, index)
    }

    pub fn start_real_node(&mut self, config: NodeStartupConfig) -> PrometheusRealNode {
        let index = self.next_index;
        self.next_index += 1;
        let node = PrometheusRealNode::start(config, index, self.host_node_parent_dir.clone());
    ...
    pub fn start_named_real_node(
        ...
        PrometheusRealNode::start_prepared(name, config, index, self.host_node_parent_dir.clone())
    }

    pub fn start_mock_node_with_real_cryptde(&mut self, ports: Vec<u16>) -> PrometheusMockNode {
    ...
    fn start_mock_node(
        ...
        let node = match public_key_opt {
            Some(public_key) => PrometheusMockNode::start_with_public_key(
                ports,
                index,
    ...
    pub fn stop(self) {
        PrometheusNodeCluster::cleanup().unwrap()
    }
    ...
    pub fn get_real_node_by_name(&self, name: &str) -> Option<PrometheusRealNode> {
    ...
    pub fn get_real_node_by_key(&self, key: &PublicKey) -> Option<PrometheusRealNode> {
        match self
            .real_nodes
            .values()
            .into_iter()
    ...
    pub fn get_mock_node_by_name(&self, name: &str) -> Option<PrometheusMockNode> {
    ...
    pub fn get_node_by_name(&self, name: &str) -> Option<Box<dyn PrometheusNode>> {
        match self.real_nodes.get(name) {
    ...
    pub fn get_real_node_home_dir_path_by_name(&self, name: String) -> String {
        PrometheusRealNode::node_home_dir(
            &self
                .host_node_parent_dir
                .clone()
                .unwrap_or_else(PrometheusNodeUtils::find_project_root),
            &name,
        )
    }
    ...
    fn cleanup() -> Result<(), String> {
        PrometheusNodeCluster::stop_running_nodes()?;
        if Self::is_in_jenkins() {
            Self::disconnect_network()
        }
        PrometheusNodeCluster::remove_network_if_running()
    }
