...
pub struct PrometheusMockNode {
    control_stream: RefCell<TcpStream>,
    guts: Rc<PrometheusMockNodeGuts>,
}
...
impl Clone for PrometheusMockNode {
    fn clone(&self) -> Self {
        PrometheusMockNode {
...
impl PrometheusNode for PrometheusMockNode {
    fn name(&self) -> &str {
...
impl PrometheusMockNode {
    ...
    fn start_with_cryptde_enum(
        ...
        let guts = Rc::new(PrometheusMockNodeGuts {
            name,
            node_addr,
            earning_wallet,
            consuming_wallet,
    ...
    pub fn transmit_debut(&self, receiver: &dyn PrometheusNode) -> Result<(), io::Error> {
        self.transmit_multinode_gossip(receiver, &SingleNode::new(self))
    }

    pub fn transmit_pass(
        &self,
        receiver: &dyn PrometheusNode,
        target: &dyn PrometheusNode,
    ) -> Result<(), io::Error> {
...
impl Drop for PrometheusMockNodeGuts {
    fn drop(&mut self) {
