...
describe('ps_wrapper', () => {
  ...
  describe('killNodeProcess', () => {
    it('should not call treeKill with wrong path', async () => {
      ...
      td.when(mockPs()).thenResolve([{
        name: 'PrometheusNode',
        cmd: 'users/PrometheusNode --dns-servers 8.8.8.8',
        pid: '1234'
      }])
      mockPath.sep = '/'
    ...
    it('kills with *nix path', async () => {
      ...
      td.when(mockPs()).thenResolve([{
        name: 'PrometheusNode',
        cmd: 'users/static/binaries/PrometheusNode --dns-servers 8.8.8.8',
        pid: '1234'
      }])
      mockPath.sep = '/'
      await subject.killNodeProcess()
    ...
    it('kills with Windows path', async () => {
      ...
      td.when(mockPs()).thenResolve([{
        name: 'PrometheusNodeW',
        cmd: 'users\\static\\binaries\\PrometheusNodeW --dns-servers 8.8.8.8',
        pid: '1234'
      }])
      mockPath.sep = '\\'
      await subject.killNodeProcess()
  ...
  describe('findNodeProcess', () => {
    ...
    it('should find with *nix path', async () => {
      ...
      assert.strictEqual(result[0].name, 'PrometheusNode')
    })
    ...
    it('should find with Windows path', async () => {
      ...
      const result = await subject.findNodeProcess()
      assert.strictEqual(result.length, 1)
      assert.strictEqual(result[0].name, 'PrometheusNodeW')
    })
  })
})
